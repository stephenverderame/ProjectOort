...
use crate::graphics_engine::{entity, instancing, shader, textures};
...
impl Drawable for Minimap {
    fn render_args<'a>(
        ...
        let attribs: Vec<_> = self
            .blips
            .iter()
            .map(|b| instancing::ParticleAttributes {
                color: b.color,
                tex_idx: b.tex_index as u32,
            })
            .collect();

        let positions: Vec<[[f32; 4]; 4]> = self
            .blips
            .iter()
            .map(|b| b.pos.mat().cast().unwrap().into())
            .collect();
        ...
        let uniform = shader::UniformInfo::Minimap(shader::MinimapData {
            textures: [&self.textures[0], &self.textures[1], &self.textures[2]],
        });

        let inst_attribs: vertex::VerticesSource<'a> = From::from(
            self.instances
                .get_stored_buffer()
                .unwrap()
                .per_instance()
        ...
        let inst_pos: vertex::VerticesSource<'a> = From::from(
            self.instance_pos
                .get_stored_buffer()
                .unwrap()
                .per_instance()
