...
lazy_static! {
    static ref SHARED_BODIES : RaceyCell<HashMap<usize, SharedRigidBody>> = RaceyCell::new(HashMap::new());
}
...
impl SharedRigidBody {
    ...
    fn new(collider: &collisions::CollisionObject) -> Self {
        unsafe { SHARED_BODIES.borrow_mut() }.entry(INVALID_SHARED_BODY_ID).or_insert_with(|| Self {
    ...
    fn get_ptr_id(collider: &Option<collisions::CollisionObject>) -> usize {
        if let Some(collider) = collider {
            ...
            unsafe { SHARED_BODIES.borrow_mut() }.entry(id)
...
impl BaseRigidBody {
        ...
        pub fn density(&mut self, density: f64) {
            ...
            let mass = self.collider.as_ref()
                .map(|collider| density * collider.aabb_volume() * scale).unwrap_or(density);
            self.mass = mass;
        }
