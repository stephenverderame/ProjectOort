...
impl DepthRenderTarget {
    ...
    fn get_fbo_rbo<'b>(
        ...
        framebuffer::SimpleFrameBuffer<'b>,
        Pin<Box<texture::DepthTexture2d>>,
    ) {
...
impl RenderTarget for DepthRenderTarget {
    fn draw(
        ...
        if let Some((ref mut fbo, depth, facs)) = self.trans_fbo.as_mut() {
            ...
            tex = TextureType::Multi(vec![
                tex,
                TextureType::Depth2d(Ref(&*depth)),
                TextureType::Tex2d(Ref(&*facs)),
            ]);
        }
        if self.render_cascades {
            Some(TextureType::WithArg(
                Box::new(tex),
                StageArgs::Cascade(vp, viewer.view_dist().1),
            ))
        } else {
            Some(tex)
        }
...
impl CubemapRenderBase {
    ...
    fn get_target_up() -> [(cgmath::Point3<f32>, cgmath::Vector3<f32>); 6] {
        ...
        [
            ...
            (point3(-1., 0., 0.), vec3(0., -1., 0.)),
            (point3(0., 1., 0.), vec3(0., 0., 1.)),
            (point3(0., -1., 0.), vec3(0., 0., -1.)),
            (point3(0., 0., 1.), vec3(0., -1., 0.)),
            (point3(0., 0., -1.), vec3(0., -1., 0.)),
        ]
    }
    ...
    fn bind_views(&self) -> camera::StaticCamera {
        ...
        for ((target, up), mat_dst) in target_faces
            .iter()
...
impl CubemapRenderTarget {
    ...
    pub fn new<F: glium::backend::Facade>(
        ...
        let cbo_tex = Box::pin(
            texture::Cubemap::empty_with_format(
                facade,
        ...
        let fbo = unsafe {
            framebuffer::SimpleFrameBuffer::with_depth_buffer(
                facade,
                (*color_ptr).main_level(),
                (*depth_ptr).main_level(),
            )
            .unwrap()
        };
        CubemapRenderTarget {
    ...
    fn draw(
        ...
        for mip_level in 0..self.mip_levels {
            ...
            let mut fbo = framebuffer::SimpleFrameBuffer::with_depth_buffer(
                &*ctx.ctx.borrow(),
                cbo_tex.mipmap(mip_level).unwrap(),
                depth_tex.mipmap(mip_level).unwrap(),
            )
            .unwrap();
            func(
                &mut fbo,
                &cam_base,
                RenderPassType::LayeredVisual,
                cache,
                TargetType::Mipcube,
                &pipeline_inputs,
            );
...
impl<V: Viewer, F: Fn(&dyn Viewer) -> V, Rt: RenderTarget>
    CustomViewRenderTargetDecorator<V, F, Rt>
{
...
impl<V: Viewer, F: Fn(&dyn Viewer) -> V, Rt: RenderTarget> RenderTarget
    for CustomViewRenderTargetDecorator<V, F, Rt>
{
    fn draw(
        &mut self,
        v: &dyn Viewer,
        ...
        self.target
            .draw(&(self.get_viewer)(v), pipeline_inputs, cache, func)
    }
