...
impl RenderPass {
    ...
    fn get_inputs<'b>(
        registers: &HashMap<u16, Vec<usize>>,
        saved_textures: &'b [TextureType<'b>],
        node: u16,
    ) -> Option<Vec<&'b TextureType<'b>>> {
        registers.get(&node).map(|input_indices| {
            input_indices
                .iter()
                .map(|idx| &saved_textures[*idx])
                .collect::<Vec<&TextureType>>()
        })
    }
