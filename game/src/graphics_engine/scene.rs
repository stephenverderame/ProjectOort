...
use super::{cubes, entity, pipeline, shader};
...
pub trait AbstractScene {
    ...
    fn render(
        &mut self,
        inputs: Option<&Vec<pipeline::TextureType>>,
...
impl Scene {
    ...
    fn get_scene_data<'a>(
        viewer: shader::ViewerData,
        pass: shader::RenderPassType,
        ibl_maps: &'a Option<shader::PbrMaps>,
        lights: &'a Option<ssbo::Ssbo<shader::LightData>>,
        light_dir: &'a Option<cgmath::Vector3<f32>>,
    ) -> shader::SceneData<'a> {
        shader::SceneData {
            viewer,
    ...
    fn render_transparency(
        ...
        for entity in firsts
            .into_iter()
            .chain(map.into_iter().map(|(_, entity)| entity))
...
impl AbstractScene for Scene {
    fn render(
        &mut self,
        _inputs: Option<&Vec<pipeline::TextureType>>,
        shader: &shader::ShaderManager,
    ) -> Option<pipeline::TextureType> {
        let vd = viewer_data_from(&*self.viewer.borrow());
        let sd = Rc::new(RefCell::new(Self::get_scene_data(
            vd,
...
pub fn compositor_scene_new<F: backend::Facade>(
            ...
            move || {
                ...
                (
                    ...
                    BlitTarget {
                        left: 0,
                        bottom: 0,
                        width: *width.borrow() as i32,
                        height: *height.borrow() as i32,
                    },
                )
            },
            |disp| {
    ...
    > AbstractScene for CompositorScene<S, SHolder, GetSHolder, CleanSHolder>
{
    fn render(
        ...
        use super::pipeline::{StageArgs, TextureProcessor, TextureType};
        use super::shader::{PipelineCache, RenderPassType, SceneData};
