...
impl Font {
    fn parse_line_to_glyph(
        ...
        for cap in param_regex.captures_iter(line.trim()) {
            let val = cap
                .get(2)
                .expect("Unable to get numeric capture group")
                .as_str()
                .parse::<i32>()
                .expect("Unable to parse value as i32");
            let key = cap
                .get(1)
                .expect("Unable to get label capture group")
                .as_str();
    ...
    fn parse_line_to_kerning(
        ...
        first.map(|first| (first, (second.unwrap(), amount.unwrap())))
    }
    ...
    fn read_from_header(header: &str) -> (i32, i32, i32, String) {
        ...
        let tex_path = Regex::new(r#"file="([a-zA-Z\.]+)""#)
            .unwrap()
            .captures(header)
            .expect("No matching file pattern found")
            .get(1)
            .expect("Unable to get file path capture group")
            .as_str();
    ...
    fn render_args<'a>(
        ...
        if self.dirty {
            ...
            let ps: Vec<_> = self
                .positions
                .iter()
                .map(|x| mat_to_instance_pos(&x.mat()))
                .collect();
            self.instance_pos.update_buffer(&ps, &*ctx);
            self.dirty = false;
        }

        let attribs: glium::vertex::VerticesSource<'a> = From::from(
            self.instances
                .get_stored_buffer()
                .unwrap()
                .per_instance()
        ...
        let locs: glium::vertex::VerticesSource<'a> = From::from(
            self.instance_pos
                .get_stored_buffer()
                .unwrap()
                .per_instance()
...
impl AbstractEntity for Text {
    ...
    fn should_render(&self, pass: shader::RenderPassType) -> bool {
        matches!(pass, shader::RenderPassType::Visual)
    }
