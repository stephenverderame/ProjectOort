...
vec2 getInterval(vec3 project_on_l, vec3 signed_dists, uvec3 vertices) {
    return putInOrder(vec2(getT(project_on_l, signed_dists, vertices.x, vertices.y),
...
void main() {
    ...
    if (location.x < a_triangles.length() && location.y < b_triangles.length()) {
        ...
        if (!(test_a || test_b) && !coplanar) {
            ...
            if (a_t.x - eps <= b_t.x && a_t.y + eps >= b_t.x || a_t.x - eps <= b_t.y && a_t.y + eps >= b_t.y ||
                b_t.x - eps <= a_t.x && b_t.y + eps >= a_t.x || b_t.x - eps <= a_t.y && b_t.y + eps >= a_t.y) {
