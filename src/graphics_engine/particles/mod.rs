...
pub fn simple_emitter<G, S, F>(pos: cgmath::Point3<f64>, are_lights: bool, particle_num: u32,
    ...
    where G : Fn(&node::Node) -> Particle + 'static, 
        S : FnMut(&mut Particle, f64) + 'static, 
        F : glium::backend::Facade
{
    use std::time::Instant;
    use cgmath::*;
    Box::new(ParticleEmitter::new(node::Node::default().pos(pos), emitter_lifetime, particle_num, facade, 
    particle_gen,
    |particle| {
        Instant::now().duration_since(particle.birth) > particle.lifetime
    },
    ...
    -> Box<dyn Emitter>
{
    ...
    let mut rnd = rand::thread_rng();
    simple_emitter(body_pos, true, rnd.gen_range(10 .. 50), Some(std::time::Duration::from_millis(10)), facade,
        move |origin| {
            let mut rnd = rand::thread_rng();
